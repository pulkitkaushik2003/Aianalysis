...
public class FileParserService {
    ...
    public void streamFile(String filePath, Consumer<Map<String, Object>> consumer) throws IOException {
        ...
        if ("csv".equals(ext)) {
            try (CSVReader reader = new CSVReader(new FileReader(filePath))) {
                ...
                while ((row = reader.readNext()) != null) {
                    if (headers == null) {
                        headers = Arrays.stream(row).map(s -> s != null ? s.trim() : "").toArray(String[]::new);
                        continue;
                    }
                    Map<String, Object> rowMap = new LinkedHashMap<>();
                    for (int i = 0; i < headers.length; i++) {
                        String value = i < row.length ? row[i].trim() : null;
                        rowMap.put(headers[i], (value != null && !value.isEmpty()) ? value : null);
                    }
                    consumer.accept(rowMap);
                }
            } catch (CsvValidationException e) {
                throw new IOException("CSV stream error: " + e.getMessage(), e);
            }
        } else if ("xlsx".equals(ext) || "xls".equals(ext)) {
            boolean isOld = "xls".equals(ext);
            try (InputStream is = Files.newInputStream(Paths.get(filePath));
                 Workbook workbook = isOld ? new HSSFWorkbook(is) : new XSSFWorkbook(is)) {
                ...
                for (int i = 1; i <= sheet.getLastRowNum(); i++) {
                    ...
                    for (int j = 0; j < headers.size(); j++) {
                        Cell cell = row.getCell(j);
                        String cellValue = (cell == null) ? null : getCellValue(cell, headers.get(j), formatter, evaluator);
                        String value = (cellValue != null) ? cellValue.trim() : null;
                        rowMap.put(headers.get(j), (value != null && !value.isEmpty()) ? value : null);
                    }
                    consumer.accept(rowMap);
                }
